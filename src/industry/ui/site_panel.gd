...
func refresh() -> void:
    ...
    match _kind:
        "mine":
            ...
            _body.text = "Niveau %d\nProduction : %.1f / min\n%s" % [game.mine_levels[_id], game.mine_rate(_id) * 60.0, _cost(game.mine_upgrade_cost(_id))]
        "drill":
            _title.text = "Foreuse"
            var reason: String = str(game.excavation_block_reason())
        ...
        "site":
            ...
            _body.text = "Profondeur : −%d m\nCapacité : %d\nÉtat : %s" % [int(site.get("depth", 0)), int(site.get("capacity", 0)), "Actif" if bool(site.get("active", false)) else "Inactif"]
