...
const PATH := "user://tests/visual_v07_captures.json"
...
func _run() -> void:
    ...
    _renderer = _screen.find_child("MineAssetRenderer", true, false)
    t.check(_world != null, "MineWorld disponible pour les captures v0.7")
    t.check(_context != null, "panneau contextuel disponible pour les captures v0.7")
    t.check(_presenter != null, "présentateur disponible pour les captures v0.7")
    t.check(_renderer != null, "MineAssetRenderer actif pour les captures v0.7")
    t.check(_world.find_child("MineModuleRenderer", false, false) == null, "aucun renderer v0.6 dupliqué sous v0.7")

    await _prepare_state(150, 6)
    _world.set("zoom", 0.75)
    _world.focus_depth(0)
    _context.clear_selection()
    await _capture(Vector2i(1280, 800), folder.path_join("v07-wide-surface.png"))

    await _prepare_state(60, 3)
    _world.set("zoom", 0.75)
    _world.focus_depth(30)
    _context.clear_selection()
    await _capture(Vector2i(1280, 800), folder.path_join("v07-wide-60m.png"))

    await _prepare_state(150, 6)
    _world.set("zoom", 0.75)
    _world.focus_depth(120)
    _context.clear_selection()
    await _capture(Vector2i(1280, 800), folder.path_join("v07-wide-150m.png"))
    ...
    t.check(deep_discovery != null, "découverte profonde disponible pour la capture v0.7 sélectionnée")
    if deep_discovery != null:
        deep_discovery.pressed.emit()
        await _settle()
        t.check(_context.visible, "contexte profond visible pour la capture v0.7 sélectionnée")
        if _presenter.has_method("marker_is_emphasized"):
            t.check(_presenter.marker_is_emphasized("Discovery_120_0"), "marqueur profond v0.7 accentué")
    await _capture(Vector2i(1280, 800), folder.path_join("v07-wide-selected.png"))

    await _prepare_state(90, 4)
    _world.set("zoom", 0.75)
    _world.focus_depth(78)
    _context.clear_selection()
    await _capture(Vector2i(720, 1000), folder.path_join("v07-narrow-90m.png"))

    _screen.queue_free()
    await process_frame
    _cleanup()
    print("Visual v0.7 captures: %s" % ("PASS" if t.failures == 0 else "FAIL"))
    quit(t.finish())
...
func _capture(dimensions: Vector2i, path: String) -> void:
    ...
    t.check(not picture.is_empty(), "capture v0.7 non vide : %s" % path.get_file())
    t.equal(picture.save_png(path), OK, "capture v0.7 enregistrée : %s" % path.get_file())
