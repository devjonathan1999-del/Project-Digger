...
const PATH := "user://tests/visual_v05_captures.json"
...
func _run() -> void:
    ...
    t.check(_world != null, "MineWorld disponible pour les captures v0.5")
    t.check(_context != null, "panneau contextuel disponible pour les captures v0.5")
    ...
    _context.call("clear_selection")
    await _settle()
    await _capture(Vector2i(1280, 800), folder.path_join("v05-wide-150m.png"))
    ...
    _context.call("clear_selection")
    await _settle()
    await _capture(Vector2i(720, 1000), folder.path_join("v05-narrow-90m.png"))

    await _prepare_state(150, 6)
    _world.set("zoom", 0.75)
    _world.focus_depth(120)
    await _settle()
    var discovery = _screen.find_child("Discovery_120_0", true, false) as Button
    t.check(discovery != null, "découverte profonde disponible pour la capture sélectionnée")
    if discovery != null:
        discovery.pressed.emit()
        await _settle()
        t.check(_context.visible, "panneau contextuel visible après sélection")
        if _presenter.has_method("marker_is_emphasized"):
            t.check(_presenter.marker_is_emphasized("Discovery_120_0"), "marqueur profond renforcé après sélection")
    await _capture(Vector2i(1280, 800), folder.path_join("v05-wide-selected.png"))

    _screen.queue_free()
    await process_frame
    _cleanup()
    print("Visual v0.5 captures: %s" % ("PASS" if t.failures == 0 else "FAIL"))
    quit(t.finish())
...
func _capture(dimensions: Vector2i, path: String) -> void:
    ...
    t.check(not picture.is_empty(), "capture v0.5 non vide : %s" % path.get_file())
    t.equal(picture.save_png(path), OK, "capture v0.5 enregistrée : %s" % path.get_file())
